...
def rewards():
        ...
        # Make sure this query is correct and the 'users' table has a 'credit_score' column
        cursor.execute('SELECT credit_score FROM users WHERE user_id = %s', (user_id,))
        user = cursor.fetchone()

        # Make sure the 'rewards' table exists
        cursor.execute('SELECT * FROM rewards WHERE is_active = TRUE ORDER BY cost ASC')
        rewards_list = cursor.fetchall()

        cursor.close()
        conn.close()

        # Make sure the variable names here ('user' and 'rewards') match the template
        return render_template('rewards.html', user=user, rewards=rewards_list)
